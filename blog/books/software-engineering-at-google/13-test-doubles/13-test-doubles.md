...
مشکلات این کد بر اساس بحث قبلی درباره خوانایی باید واضح باشد. اول اینکه، اگرچه بدنه تست‌ها بسیار مختصر هستند، کامل نیستند: جزئیات مهم در metohd‌های کمکی (Helper Methods) پنهان شده‌اند که خواننده بدون پیمایش به بخش کاملاً متفاوتی از فایل نمی‌تواند آن‌ها را ببیند. این metohd‌های کمکی نیز پر از منطق هستند که بررسی آن‌ها را در یک نگاه دشوار می‌کند (آیا باگ را پیدا کردید؟). تست زمانی بسیار واضح‌تر می‌شود که با استفاده از اصول DAMP (Don't Abstract Much/Practically) بازنویسی شود، همان‌طور که در Example 12-20 نشان داده شده است.

Example 12-20. Testها باید DAMP باشند
در این مثال، هر تست یک رفتار واحد را آزمایش می‌کند و setup و assertion‌های خوانا دارد. کد تست مستقیماً قابل درک است و نیازی به دنبال کردن metohd‌های کمکی در بخش‌های دیگر فایل نیست.
...
با این حال، metohd‌های اعتبارسنجی متمرکزتر همچنان می‌توانند مفید باشند. بهترین metohd‌های کمکی اعتبارسنجی، یک واقعیت مفهومی واحد درباره ورودی‌های خود را تأیید می‌کنند، در مقابل metohd‌های اعتبارسنجی عمومی که طیف وسیعی از شرایط را پوشش می‌دهند. چنین metohd‌هایی زمانی می‌توانند به‌طور خاص مفید باشند که شرایطی که اعتبارسنجی می‌کنند از نظر مفهومی ساده باشد اما برای پیاده‌سازی نیاز به حلقه یا منطق شرطی داشته باشد که اگر در بدنه metohd تست قرار می‌گرفت، خوانایی را کاهش می‌داد. به عنوان مثال، metohd کمکی در Example 12-25 ممکن است در تستی که چندین حالت مختلف درباره دسترسی به حساب را پوشش می‌دهد، مفید باشد.

**تعریف Test Infrastructure (زیرساخت تست)**

تکنیک‌هایی که تاکنون بحث کردیم، اشتراک‌گذاری کد بین metohd‌ها در یک کلاس یا suite تست واحد را پوشش می‌دهند. گاهی اوقات، اشتراک‌گذاری کد بین چندین suite تست نیز می‌تواند ارزشمند باشد. ما به این نوع کد، test infrastructure یا زیرساخت تست می‌گوییم. اگرچه معمولاً در تست‌های integration یا end-to-end ارزشمندتر است، زیرساخت تستی که به‌دقت طراحی شده باشد می‌تواند در برخی شرایط نوشتن تست‌های واحد را بسیار آسان‌تر کند.

زیرساخت تست سفارشی باید با دقت بیشتری نسبت به اشتراک‌گذاری کدی که در یک suite تست واحد اتفاق می‌افتد، مدیریت شود. از بسیاری جهات، کد زیرسYSTS تست بیشتر شبیه کد production است تا سایر کد تست، با توجه به اینکه می‌تواند فراخوان‌کنندگان زیادی داشته باشد که به آن وابسته‌اند و ممکن است بدون ایجاد شکستگی، تغییر دادنش دشوار باشد. از بیشتر مهندسان انتظار نمی‌رود هنگام تست کردن featureهای خود، تغییراتی در زیرساخت تست مشترک ایجاد کنند. زیرسYSTS تست باید به عنوان یک محصول جداگانه مستقل مدیریت شود و بر این اساس، زیرساخت تست همیشه باید تست‌های خود را داشته باشد.

البته، بیشتر زیرساخت تستی که اکثر مهندسان از آن استفاده می‌کنند، به شکل کتابخانه‌های شخص ثالث (Third-Party Libraries) شناخته‌شده مانند JUnit ارائه می‌شود. تعداد زیادی از این کتابخانه‌ها موجود است و استانداردسازی آن‌ها در سازمان باید تا حد ممکن زود و به‌صورت جهانی اتفاق بیفتد. به عنوان مثال، گوگل سال‌ها پیش Mockito را به عنوان تنها mocking framework (چارچوب شبیه‌سازی) که باید در تست‌های جدید Java استفاده شود، اجباری کرد و استفاده از سایر mocking frameworkها را در تست‌های جدید ممنوع کرد. این فرمان در آن زمان باعث نارضایتی افرادی شد که با frameworkهای دیگر راحت بودند، اما امروزه به‌طور جهانی به عنوان حرکتی خوب دیده می‌شود که تست‌های ما را قابل درک‌تر و کار با آن‌ها را آسان‌تر کرد.

**تست‌ها و اشتراک‌گذاری کد: DAMP، نه DRY**
...
استفاده از یک سرویس کارت اعتباری واقعی در یک تست عملی نیست (هزینه‌های تراکنش اجرای تست را تصور کنید!)، اما می‌توان به جای آن از یک test double (جایگزین تست) استفاده کرد تا رفتار سیستم واقعی را شبیه‌سازی کند. کد در Example 13-2 یک test double بسیار ساده را نشان می‌دهد.

**Seams (درزها)**

کد زمانی testable (قابل تست) گفته می‌شود که به شکلی نوشته شده باشد که نوشتن تست‌های واحد برای آن ممکن باشد. Seam یا درز، راهی برای testable کردن کد با فراهم کردن امکان استفاده از test doubleهاست — این امکان را فراهم می‌کند که برای سیستم تحت تست (System Under Test) از وابستگی‌های متفاوتی نسبت به وابستگی‌های مورد استفاده در محیط production استفاده شود.

Dependency Injection (تزریق وابستگی) تکنیک رایجی برای معرفی seams است. به طور خلاصه، وقتی یک کلاس از dependency injection استفاده می‌کند، هر کلاسی که نیاز به استفاده از آن دارد (یعنی وابستگی‌های کلاس) به جای نمونه‌سازی مستقیم، به آن پاس داده می‌شود و این امکان را فراهم می‌کند که این وابستگی‌ها در تست‌ها جایگزین شوند.
...
**پیاده‌سازی‌های واقعی (Real Implementations)**

استفاده از پیاده‌سازی‌های واقعی به این دلیل است که کد دقیقاً همان‌طور که در production اجرا می‌شود، اجرا شود و استفاده از پیاده‌سازی‌های واقعی به تحقق این هدف کمک می‌کند.

در گوگل، ترجیح برای پیاده‌سازی‌های واقعی با گذشت زمان شکل گرفت، زیرا دیدیم که استفاده بیش از حد از mocking frameworkها تمایل به آلوده کردن تست‌ها با کد تکراری داشت که از پیاده‌سازی واقعی جدا می‌شد و refactoring (بازآرایی) را دشوار می‌کرد. این موضوع را بعداً در این فصل با جزئیات بیشتر بررسی خواهیم کرد.

**ترجیح واقعیت بر انزوا (Prefer Realism Over Isolation)**

استفاده از پیاده‌سازی‌های واقعی برای وابستگی‌ها، سیستم تحت تست را واقعی‌تر می‌کند، زیرا تمام کد در این پیاده‌سازی‌های واقعی در تست اجرا خواهد شد. در مقابل، تستی که از test doubleها استفاده می‌کند، سیستم تحت تست را از وابستگی‌هایش جدا می‌کند به طوری که تست کد در وابستگی‌های سیستم تحت تست را اجرا نمی‌کند.

ما تست‌های واقعی‌تر را ترجیح می‌دهیم زیرا اطمینان بیشتری می‌دهند که سیستم تحت تست به درستی کار می‌کند. اگر تست‌های unit بیش از حد روی test doubleها تکیه کنند، ممکن است مهندس نیاز داشته باشد تست‌های integration را اجرا کند یا به صورت دستی تأیید کند که feature مورد نظرش مطابق انتظار کار می‌کند تا به همان سطح اعتماد برسد. انجام این وظایف اضافی می‌تواند توسعه را کند کند و حتی اگر مهندسان این وظایف را به طور کلی رد کنند، باگ‌ها می‌توانند رد شوند.

جایگزینی تمام وابستگی‌های یک کلاس با test doubleها به طور دلخواه سیستم تحت تست را به پیاده‌سازی‌ای که نویسنده به‌طور تصادفی مستقیماً در کلاس قرار داده، محدود می‌کند و پیاده‌سازی‌ای که در کلاس‌های دیگر است را حذف می‌کند. با این حال، یک تست خوب باید مستقل از پیاده‌سازی باشد — باید بر اساس API مورد تست نوشته شود، نه بر اساس نحوه ساختار پیاده‌سازی.

استفاده از پیاده‌سازی‌های واقعی می‌تواند باعث شکست تست شما شود اگر باگی در پیاده‌سازی واقعی وجود داشته باشد. این خوب است! شما می‌خواهید تست‌هایتان در چنین مواردی شکست بخورند زیرا نشان می‌دهد که کد شما در production به درستی کار نخواهد کرد. گاهی اوقات، یک باگ در پیاده‌سازی واقعی می‌تواند باعث شکست زنجیره‌ای تست‌ها شود زیرا تست‌های دیگری که از پیاده‌سازی واقعی استفاده می‌کنند نیز ممکن است شکست بخورند. اما با ابزارهای توسعه خوب، مانند
...
**چرا Fakeها مهم هستند؟**

Fakeها می‌توانند ابزار قدرتمندی برای تست باشند: سریع اجرا می‌شوند و به شما اجازه می‌دهند کد خود را به‌طور مؤثر بدون معایب استفاده از پیاده‌سازی‌های واقعی تست کنید.

یک fake تنها می‌تواند تجربه تست کردن یک API را به طرز چشمگیری بهبود بخشد. اگر این را به تعداد زیادی fake برای انواع مختلف APIها مقیاس دهید، fakeها می‌توانند بهبود عظیمی در سرعت مهندسی (Engineering Velocity) در سراسر سازمان نرم‌افزاری ایجاد کنند.

در سمت دیگر طیف، در سازمان نرم‌افزاری که fakeها نادر هستند، سرعت کمتر خواهد بود زیرا مهندسان ممکن است در استفاده از پیاده‌سازی‌های واقعی که منجر به تست‌های کند و شکننده (Flaky Tests) می‌شوند، به مشکل بخورند. یا مهندسان ممکن است به تکنیک‌های دیگر test double مانند stubbing یا interaction testing (تست تعاملی) متوسل شوند که همان‌طور که بعداً در این فصل بررسی خواهیم کرد، می‌تواند منجر به تست‌هایی نامشخص، شکننده و کم‌اثرتر شود.

**چه زمانی Fake نوشته شود؟**

یک fake به تلاش و تجربه حوزه‌ای (Domain Experience) بیشتری برای ایجاد نیاز دارد زیرا باید مشابه پیاده‌سازی واقعی رفتار کند. یک fake همچنین نیاز به نگهداری (Maintenance) دارد: هر زمان رفتار پیاده‌سازی واقعی تغییر می‌کند، fake نیز باید به‌روز شود تا با این رفتار مطابقت داشته باشد. به همین دلیل، تیمی که مالک پیاده‌سازی واقعی است باید fake را بنویسد و نگهداری کند.

اگر تیمی در حال بررسی نوشتن fake است، باید تصمیم بگیرد که آیا بهبود بهره‌وری حاصل از استفاده fake، هزینه‌های نوشتن و نگهداری آن را توجیه می‌کند یا خیر. اگر فقط تعداد کمی کاربر وجود داشته باشد، ممکن است ارزش وقت آن‌ها را نداشته باشد، در حالی که اگر صدها کاربر وجود داشته باشد، می‌تواند منجر به بهبود آشکار بهره‌وری شود.

برای کاهش تعداد fakeهایی که نیاز به نگهداری دارند، معمولاً fake باید فقط در ریشه کدی ایجاد شود که استفاده از آن در تست‌ها عملی نیست. به عنوان مثال، اگر یک پایگاه داده (Database) نمی‌تواند در تست‌ها استفاده شود، fake باید برای خود API پایگاه داده وجود داشته باشد، نه برای هر کلاسی که API پایگاه داده را فراخوانی می‌کند.

نگهداری fake می‌تواند دشوار باشد اگر پیاده‌سازی آن نیاز به تکرار در زبان‌های برنامه‌نویسی مختلف داشته باشد، مانند سرویسی که client libraryهایی دارد که اجازه فراخوانی سرویس از زبان‌های مختلف را می‌دهند. یک راه حل برای این حالت، ایجاد یک پیاده‌سازی fake service واحد و پیکربندی client libraryها توسط تست‌ها برای ارسال درخواست‌ها به این fake service است. این رویکرد در مقایسه با نوشتن کاملاً fake در حافظه، سنگین‌تر است زیرا نیاز دارد تست‌ها از طریق processها ارتباط برقرار کنند. با این حال، می‌تواند trade-off منطقی باشد، تا زمانی که تست‌ها همچنان بتوانند سریع اجرا شوند.
